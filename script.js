...
if (districtTitle && selectedRegion && selectedDistrict) {
  ...
  document.querySelector('meta[name="description"]')?.setAttribute('content', `${fullName} 학생을 위한 국제학교·수학·영어 맞춤 과외 안내입니다. 방문·화상 수업과 무료 상담을 제공합니다.`);
  ...
  document.getElementById('district-intro').textContent = `${fullName}의 학교 일정과 생활 패턴을 고려해 학생에게 맞는 방문·화상 과외와 학습 코칭을 안내합니다.`;
  document.getElementById('breadcrumb-district').textContent = selectedDistrict;
  document.getElementById('plan-title').textContent = `${selectedDistrict} 맞춤 학습 플랜`;
  document.getElementById('stat-copy').textContent = `${selectedDistrict} 학생의 학년과 목표에 맞춘 1:1 상담을 진행합니다.`;
